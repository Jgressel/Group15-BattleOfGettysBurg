...
;  ]
  ...
  create-confederate-army (number-of-confederate-soldiers * 0.9965) [
    ...
    let x-coordinate ifelse-value (random 2 = 0) [ -7 + random-float 2 - .25 ] ifelse-value (random 2 = 0)[ -5 + random-float 2 - .25 ] [ -8 + random-float 2 - .25 ]
    let y-coordinate 30 + random-float 1 - .5
  ...
  create-union-army (number-of-union-soldiers * 0.99) [
    ...
    ;set xcor -4 + random-float 3 - 0.5
    ;set ycor 28 + random-float 2 - 0.5
    setxy x-coordinate y-coordinate
  ]
...
"" ""
PENS
"Union" 1.0 0 -14070903 true "" "if (number-of-union-soldiers - count union-army) * (turtle-troop-size) > 0 [ plot (number-of-union-soldiers - count union-army) * (turtle-troop-size) ]"
